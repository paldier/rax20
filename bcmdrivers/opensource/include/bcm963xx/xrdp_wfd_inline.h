...
static inline struct sk_buff *single_packet_read_and_handle(uint32_t qid, wfd_object_t *wfd_p, uint16_t *tx_prio)
{
    rdpa_cpu_rx_info_t info = {};
    ...
    if (unlikely(rdpa_cpu_packet_get(rdpa_cpu_wlan0 + wfd_p->wl_radio_idx, qid, &info)))
       return NULL;
...
static uint32_t wfd_bulk_skb_get(unsigned long qid, unsigned long budget, void *priv)
{
    ...
    while (budget)
    {
        ...
        budget--;
        if (!(skb = single_packet_read_and_handle(qid, wfd_p, &pktlist_prio)))
...
static uint32_t wfd_bulk_skb_get(unsigned long qid, unsigned long budget, void *priv)
{
    ...
    while (budget)
    {
        ...
        budget--;
        if (!(skb = single_packet_read_and_handle(qid, wfd_p, &dummy)))
            continue; // exception handled or allocation failure
