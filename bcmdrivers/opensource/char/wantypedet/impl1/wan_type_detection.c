...
#define GET_GPON_REG_ADDRESS(physical_address) ( (unsigned int *)( (physical_address) - GPON_PHYS_BASE + \
    bcm_io_block_address[GPON_IDX] ) )
#else
#define GET_GPON_REG_ADDRESS(physical_address) ( (unsigned int *)( (physical_address) - XRDP_PHYS_BASE + \
    bcm_io_block_address[XRDP_IDX] ) )
#endif
...
#define NGPON_RX_GEN_SYNC_CFG_CLEAR_MASK            (    ~0xFFF)
    ...
    reg_address = (unsigned int *)(NGPON_RX_GEN_RCVRCFG_REG_ADDRESS - NGPON2_PHYS_BASE +
        bcm_io_block_address[NGPON2_IDX]);
    *reg_address = GPON_RX_GEN_RCVRCFG_MAC_MODE_RESET;
    
    msleep(SLEEP_IN_MILI_SECONDS);

    reg_address = (unsigned int *)(NGPON_RX_GEN_SYNC_CFG_REG_ADDRESS - NGPON2_PHYS_BASE +
        bcm_io_block_address[NGPON2_IDX]);
    reg_value = *reg_address;
    ...
    *reg_address = reg_value;
    ...
    reg_value = *(unsigned int *)(NGPON_RX_GEN_RCVRSTAT_REG_ADDRESS - NGPON2_PHYS_BASE +
        bcm_io_block_address[NGPON2_IDX]);
