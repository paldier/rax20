...
static int port_proc_cmd_if_dn_phy_pwr(int argc, char *argv[])
{
    if (argc == 1)
        printk("interface down PHY power = %s\n", g_enet_flags & ENET_FLAG_IF_DOWN_PHY_UP ? "up" : "down");
    else if (!strcmp(argv[1], "up"))
        g_enet_flags |= ENET_FLAG_IF_DOWN_PHY_UP;
...
Error:
    printk("Usage: if_dn_phy_pwr <up|down>        set global interface down PHY power behavior\n"
           "       if_dn_phy_pwr                  get current global behavior\n");
    return 0;
}
...
static struct proc_cmd_ops command_entries[] = {
    ...
    { .name = "if_dn_phy_pwr",   .do_command = port_proc_cmd_if_dn_phy_pwr},
};
