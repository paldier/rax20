	...
	if (nvram_match("broadstream_iqos_wan_bw_auto", "1")) {
		...
		do {
			...
			eval("curl", "-s", "-o", "/tmp/speedtest-test",
				//"http://c.speedtest.net/speedtest-config.php?x=5p33dy");
		...
		system("cat /tmp/speedtest_download_result");
		...
		if (strncmp(line, "ceil_down=", strlen("ceil_down=")) != 0) {
			fprintf(stderr, "incorrect content 1: /tmp/speedtest_download_result\n");
			fclose(fp1);
		...
		if (download_len >= sizeof(download)) {
			fprintf(stderr, "incorrect content 2: /tmp/speedtest_download_result\n");
			fclose(fp1);
			eval("rm", "/tmp/speedtest_download_result");
            return -1;
            //goto bypass_autowan;
		...
		if (strncmp(line, "ceil_up=", strlen("ceil_up=")) != 0) {
			fprintf(stderr, "incorrect content 1: /tmp/speedtest_upload_result\n");
			fclose(fp1);
		...
		if (upload_len >= sizeof(upload)) {
			fprintf(stderr, "incorrect content 2: /tmp/speedtest_upload_result\n");
			fclose(fp1);
			eval("rm", "/tmp/speedtest_upload_result");
            return -1;
            //goto bypass_autowan;
...
bypass_autowan:
    ...
    if (strcmp(argv[1], "start") == 0) {
	    ...
	    {
	#if !defined(BCA_HNDROUTER)
	        //eval(PATH_SETUP_SH, "start", wan_ifname, lan_ifname);
	        eval(PATH_SETUP_SH, "restart", wan_ifname, lan_ifname);
	#else
	        eval(PATH_SETUP_SH, "start", wan_ifname, lan_ifname);
	#endif
	        //printf("\n\t %s(%d) bcmiqosd %s start\n", __func__, __LINE__, PATH_QOS_SH);
