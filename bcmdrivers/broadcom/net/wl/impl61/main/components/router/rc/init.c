 ...
 *
 * $Id: init.c 767578 2018-09-18 02:51:36Z $
 */
...
shutdown_system(void)
{
	...
	{
		/* XXX We use dongle watchdog only to detect host reboot so we turn it on
		 * here. Dongle watchdog is based on high driver watchdog polling and could
		 * be delayed when host is busy. We don't want it to fire just because host
		 * is too busy to poll dongle.
		 */
		int ret = 0, i = 0;
		char	ifname[16] = {0};
