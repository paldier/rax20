 ...
 *
 * $Id: wps_pbcd.c 776708 2019-07-08 06:19:00Z $
 */
...
#define HAPD_WPS_MAX_BRIDGE_NUM 5
...
/* Added start by foxconn */
#define WPS_STATE_INPROGRESS    "0"
#define WPS_STATE_SUCCESS_STOP  "1"
#define WPS_STATE_FAIL_STOP     "2"
#define WPS_STATE_TIMEOUT_STOP  "3"
#define WPS_STATE_OVERLAP_STOP  "4"
/* Added end by foxconn */
...
wps_pbcd_led_and_ui_status_update_handler(wps_pbcd_info_t *pbcd_info)
{
	...
	if (strstr(data, WPS_EVENT_ACTIVE)) {	// PBC is active
		...
		nvram_set("wps_state", WPS_STATE_INPROGRESS);   /*foxconn*/
	} else if (strstr(data, WPS_EVENT_SUCCESS)) {	// Wps is successful
		idx = WLIF_WPS_UI_OK;
		blink_type = WLIF_WPS_BLINKTYPE_STOP;
		nvram_set("wps_state", WPS_STATE_SUCCESS_STOP);   /*foxconn*/
		...
		nvram_set("wps_state", WPS_STATE_OVERLAP_STOP);   /*foxconn*/
	} else if (strstr(data, WPS_EVENT_TIMEOUT)) {	// Wps session timedout
		idx = WLIF_WPS_UI_TIMEOUT;
		blink_type = WLIF_WPS_BLINKTYPE_STOP;
		nvram_set("wps_state", WPS_STATE_TIMEOUT_STOP);   /*foxconn*/
	} else if (strstr(data, WPS_EVENT_FAIL)) { // Wps session failed
		char *pin_validation_err_token = "config_error=18";

		idx = WLIF_WPS_UI_ERR;
		blink_type = WLIF_WPS_BLINKTYPE_STOP;

		// Check if wps failure is due to the pin validation error
		if (strstr(data, pin_validation_err_token)) {
			char *ptr, *mac_token = "peer_macaddr=", *msg_token = "msg=8";

			if (strstr(data, msg_token)) {
		...
		nvram_set("wps_state", WPS_STATE_FAIL_STOP);   /*foxconn*/
	}
...
wps_pbcd_wpa_ctrl_interface_monitor(void *arg)
{
	...
	cprintf("Info: wps_pbcd thread is running for %s\n", pbcd_info->wpa_ctrl_dir);
...
exit:
	cprintf("Info: wps_pbcd thread is exiting for %s\n", pbcd_info->wpa_ctrl_dir);
...
wps_pbcd_poll_button_press()
{
	...
	while (1) {
		...
		if (strcmp(wps_on_sta, "1") == 0) {
			...
			// (120 seconds) hence register for WPS_TIMEOUT notification so that after
			// timeout duration is over wps led can be stopped.
			if (wps_ifnames && press > WPS_PBCD_BUTTON_PRESS_TYPE_UNKNOWN &&
	...
	cprintf("Info: wps_pbcd %s %d exiting\n", __func__, __LINE__);
	return;
}
...
wps_pbcd_stop_wps(void *arg)
{
	...
	foreach(ifname, wps_ifnames, next) {
		cprintf("Info: wps_pbcd %s stoping wps session on ifname %s \n", __func__, ifname);
