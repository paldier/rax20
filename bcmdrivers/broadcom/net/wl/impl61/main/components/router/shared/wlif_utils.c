...
/* FXCN add, send debug message to telnet */
void telnet_printf(char *content)
{
    char *current_tty = NULL;
    int ttyfd = 0;

    ttyfd = open("/dev/ttyp0", O_RDWR);
    write(ttyfd, content, strlen(content));
...
wl_wlif_wps_pbc_hdlr(char *wps_ifname, char *bh_ifname)
{
	...
	int ret = -1;
        ...
        if (nvram_match("TE_TEST_mode","1")) {
            fprintf(stderr, "[WPS] button clicked. \n");
            telnet_printf("[WPS] button clicked.\n");
            return 0;
        }
