...
/* Foxconn add start, Max Ding, 07/15/2017 */
int g_hop_count = 0;
int g_need_update_hop = 0;
extern int g_stop_scan;
/* Foxconn add end, Max Ding, 07/15/2017 */
				...
				struct wpa_driver_scan_params *params)
{
	...
	/* Foxconn add start, Max Ding, 07/03/2019 */
	if (g_stop_scan)
		return -1;
	/* Foxconn add end, Max Ding, 07/03/2019 */
...
/* Foxconn add start, Max Ding, 05/18/2017 @NTGR loop prevention */
/* Compare function for sorting scan results. Return >0 if @b is considered
 * better. */
static int wpa_vie_compar(const void *a, const void *b)
{
#define MIN(a,b) a < b ? a : b
	struct wpa_scan_res **_wa = (void *) a;
	struct wpa_scan_res **_wb = (void *) b;
...
/* Foxconn add end, Max Ding, 05/18/2017 */
...
void max_hexdump(const char *title, const void *buf, size_t len)
{
    size_t i;
...
static void dump_max_scan_res(struct wpa_scan_results *scan_res, int max_num)
{
	size_t i;

    if (scan_res->num < max_num)
        max_num = scan_res->num;

    if (scan_res->res != NULL && max_num != 0) {
        cprintf("dump scan res %d end\n", max_num);
        ...
        for (i = 0; i < max_num; i++) {
            ...
            pos = (u8 *) (r + 1);
            cprintf("res[%d] r->ie_len %d, r->beacon_ie_len %d end\n", i, r->ie_len, r->beacon_ie_len);
            if (r->ie_len)
                max_hexdump("IEs", pos, r->ie_len);
            pos += r->ie_len;
            if (r->beacon_ie_len)
                max_hexdump("Beacon IEs",
                        pos, r->beacon_ie_len);
        }
    }
}
...
/* Foxconn add start, Max Ding, 05/18/2017 @NTGR loop prevention */
int wpa_supplicant_ssid_match(struct wpa_supplicant *wpa_s,
                      struct wpa_scan_res *res)
{
    size_t i;
    struct wpa_ssid *p_ssid;
    const u8 *ie;
    ...
    p_ssid = wpa_s->conf->ssid;

    while (p_ssid) {
        if (0 == p_ssid->ssid_len) /* empty ssid conf, connect to any AP */
            return 1;
        if ((p_ssid->ssid_len == ie[1]) && (p_ssid->ssid != NULL)) {
            //if (0 == strncmp(p_ssid->ssid, &ie[2], p_ssid->ssid_len))
            if (0 == os_memcmp(p_ssid->ssid, ie + 2, p_ssid->ssid_len))
                return 1;
        }
        p_ssid = p_ssid->next;
    }
...
const u8 * wpa_supplicant_get_ntgr_vie(struct wpa_scan_res *res)
{
    const u8 *end, *pos;
...
static int set_bssid_for_vie(struct wpa_supplicant *wpa_s,
                struct wpa_scan_results *res)
{
    size_t i, j;
    struct wpa_scan_res *p_tmp;
    struct wpa_ssid *p_ssid;
    int match_num = 0;
    int match_index = 0;
    int match_max_rssi = 0;
    unsigned int min_hop = 0;
    u8 *ie;
    FILE *fp = NULL;
    char line[32];
    unsigned int tmp_hop_count;
    int (*compar)(const void *, const void *) = wpa_scan_result_compar;

    if (g_need_update_hop) {
        //cprintf("g_need_update_hop %d, g_hop_count %d\n", g_need_update_hop, g_hop_count);
        g_need_update_hop = 0;
        fp = fopen("/tmp/cur_hop_count", "r");
        if (fp) {
            fgets(line, sizeof(line), fp);
            tmp_hop_count = atoi(line);
            fclose(fp);
            if (tmp_hop_count < 16) {
                g_hop_count = tmp_hop_count;
            }
            //cprintf("tmp_hop_count %d, g_hop_count %d\n", tmp_hop_count, g_hop_count);
        }
    }

    /* step 1: filter matched ssid and count it */
    for (i=0,j=0; i<res->num; i++) {
        if (wpa_supplicant_ssid_match(wpa_s, res->res[i])) {
            /* switch them */
            if (res->res[i]->level > match_max_rssi)
                match_max_rssi = res->res[i]->level;
            if (i != j) {
                p_tmp = res->res[i];
                res->res[i] = res->res[j];
                res->res[j] = p_tmp;
    ...
    match_num = j;
    //cprintf("set_bssid_for_vie find %d same ssids in %d ssids\n", match_num, res->num);

    if (match_num == 0)
        return 0;

    //dump_max_scan_res(res, match_num);
    /* check Beacon has NTGR VIE and Product Types is Router */
    for (i=0,j=0; i<res->num; i++) {
        if (i<match_num) {
            ie = wpa_supplicant_get_ntgr_vie(res->res[i]);
            if (ie != NULL) {
                if ((ie[1] > 7) && (ie[6] == 0x02)) {/* Product Type is Extender */
                    /* not available case: filter out the ssid, then we won't connect to the device */
                    if ((!(ie[9] & 0x10)) /* Connection status Bit4: 1 for available, 0 for not available (Router always 1 in this bit) */
                        || ((g_hop_count != 0) && ((ie[9] & 0x0f) >= g_hop_count))
                        ) { 
                        //cprintf("filter out %dth scan result, ie[9] 0x%x, g_hop_count %d, bssid: "MACSTR"\n", i+1, ie[9], g_hop_count,
                        //        MAC2STR(res->res[i]->bssid));
                        os_free(res->res[i]);
                        res->res[i] = NULL;
                        continue;
                    }
    ...
    if (res->num != j) {
        //cprintf("Filtered out %d scan results",
        //       (int) (res->num - j));
        match_num -= res->num - j;
        res->num = j;
        //cprintf("set_bssid_for_vie 2find %d same ssids in %d ssids\n", match_num, res->num);
    }

    if (match_num == 0)
        return 0;

    /* sort according to RSSI(res->level) */
    qsort(res->res, match_num, sizeof(struct wpa_scan_res *),
          compar);
    ...
    for (i=0; i<match_num; i++) {
        /* Foxconn removed by peanut xu, 12/21/2017 according to new spec "NTGR  Extender Loop prevention_170811_MT17111501.pptx" */
        //if (match_max_rssi > res->res[i]->level+10)
        //    continue;
        ie = wpa_supplicant_get_ntgr_vie(res->res[i]);
        if (ie != NULL) {
            if (ie[6] == 0x01) {/* Product Type is Router */
                //cprintf("set_bssid_for_vie find NTGR router\n");
                match_index = i;
                goto match;
            }
        }
    ...
    for (i=0; i<match_num; i++) {
        ...
        ie = wpa_supplicant_get_ntgr_vie(res->res[i]);
        if (ie == NULL) {
            //cprintf("set_bssid_for_vie find non-NTGR vie router\n");
            match_index = i;
            goto match;
        } else {
            if (ie[6] == 0x00) {/* Product Type is invalid, treat is as non-NTGR router */
                //cprintf("set_bssid_for_vie find NTGR vie(product type = 0) router\n");
                match_index = i;
                goto match;
            }
        }
    ...
    /* check Beacon has NTGR VIE and Product Types is Router */
    min_hop = 16; /* hop range 0-15 */
    match_index = 0; /* must set one BSSID, here use the best signal ssid */
    for (i=0; i<match_num; i++) {
        /* Foxconn removed by peanut xu, 12/21/2017 according to new spec "NTGR  Extender Loop prevention_170811_MT17111501.pptx" */
        //if (match_max_rssi > res->res[i]->level+10)
        //    continue;
        ie = wpa_supplicant_get_ntgr_vie(res->res[i]);
        if (ie != NULL) {
            if (ie[6] == 0x02) {/* Product Type is Extender */
                if ((ie[1] > 7) && (ie[9] & 0x10)) { /* Connection status Bit4: 1 for available, 0 for not available (Router always 1 in this bit) */
                    if ((ie[9] & 0x0f) < min_hop) {
                        //cprintf("set_bssid_for_vie find NTGR vie(Connection status Bit4: 1) extender\n");
                        min_hop = ie[9] & 0x0f;
                        match_index = i; //set the min hop bssid
                    }
                    //goto match;
                }
            }
    ...
    /* must set one BSSID, here use the best signal ssid */
    //match_index = 0;
    goto match;
...
match:
    /* set BSSID */
    cprintf("set_bssid_for_vie match index %d, bssid: "MACSTR" \n", match_index, 
                       MAC2STR(res->res[match_index]->bssid));
    p_ssid = wpa_s->conf->ssid;
    while (p_ssid) {
        p_ssid->bssid_set = 1;
        os_memcpy(p_ssid->bssid, res->res[match_index]->bssid, ETH_ALEN);
        cprintf("set_bssid_for_vie for network id %d end\n", p_ssid->id);
        p_ssid = p_ssid->next;
    }
    return 1;
}
				...
				struct scan_info *info, int new_scan)
{
	...
	/* Foxconn add start, Max Ding, 06/01/2017 @stop scan before GUI do site survey */
	if (g_stop_scan) {
		return NULL;
	}
	/* Foxconn add end, Max Ding, 06/01/2017 */
	...
	else /* Foxconn added by Max Ding, 05/18/2017 */
#endif /* CONFIG_WPS */
	{
		set_bssid_for_vie(wpa_s, scan_res); /* Foxconn added by Max Ding, 05/18/2017 @NTGR loop prevention */
	}
