...
int g_stop_scan = 0;
static void cancel_stop_scan(void *eloop_data, void *user_ctx)
{
    g_stop_scan = 0;
}
static void handle_sig_usr(int signo)
{
    if (signo == SIGUSR1)
    {
        g_stop_scan = 1;
        eloop_cancel_timeout(cancel_stop_scan, NULL, NULL);
        eloop_register_timeout(30, 0, cancel_stop_scan, NULL, NULL);
    }
    if (signo == SIGUSR2)
    {
        eloop_cancel_timeout(cancel_stop_scan, NULL, NULL);
        g_stop_scan = 0;
        extern int g_need_update_hop;
        g_need_update_hop = 1;
    }
}
...
int main(int argc, char *argv[])
{
	...
	/* Foxconn add start, Max Ding, 06/01/2017 */
