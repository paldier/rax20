 ...
 *
 * $Id: hndsoc.h 768508 2018-10-16 14:42:02Z $
 */
...
#define SI_ENUM_BASE_BP_2x2AX		0x84000000	/**< 2x2 ax core specific */
...
#define SI_ENUM_BASE_BP(sih)	SI_ENUM_BASE(sih)
 ...
 * SI_ENUM_BASE() returns a host physical (so not a host logical) address.
 */
...
/* SI_ENUM_BASE_BP() and SI_WRAP_BASE_BP() were introduced for the 2x2 ax core. These cores are
 * embedded in chips with a UBUS backplane, and only support NIC driver mode. Address translation
 * takes place in the host CPU -> WLAN core direction, which explains the need for these macro's.
 */
/** returns physical addr of chipcommon core from backplane (eg, BME) perspective */
#define SI_ENUM_BASE_BP(sih)	((sih)->enum_base_bp)
/** returns physical addr of chipcommon wrapper from backplane (eg, BME) perspective */
#define SI_WRAP_BASE_BP(sih)	(SI_ENUM_BASE_BP(sih) + 0x00100000)
#endif /* DONGLEBUILD */
