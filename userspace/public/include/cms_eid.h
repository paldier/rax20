...
typedef enum {
   ...
   EID_FXCN_HTTPD=5001,        /* Foxconn EID for httpd */
   EID_FXCN_SHARED=5002,        /* Foxconn EID for libshared.so */
   EID_FXCN_HEARTBEAT=5003,     /* Foxconn EID for heartbeat */
   EID_FXCN_BCM_UTIL=5004,     /* Foxconn EID for bcm_utils */
...
typedef enum {
   CMS_EE_INDEX_INVALID = 0,
   CMS_EE_INDEX_1 = 1,
   CMS_EE_INDEX_2 = 2
} CmsExtendEntityIndex;
 ...
 *
 * Multi-threaded apps can use this function to uniquely identify threads
 * in a multi-threaded process.
 */ 
SINT32 cmsEid_getThreadId(void);
