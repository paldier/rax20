...
static void dump_stats(spdt_stat_t *stats)
{
    if (is_recv)
    {
        ...
        sec = stats->proto_ext.udp_basic.rx_usec / 1000000;
        if (sec)
            rate = stats->proto_ext.udp_basic.rx.bytes * 8 / sec;

        printf("Stats: RX packets %lld, RX bytes %lld, usec %lld, (Goodput) Rate = ",
            stats->proto_ext.udp_basic.rx.packets,
    ...
    {
        printf("Stats: TX packets %lld, TX bytes %lld\n", stats->proto_ext.udp_basic.tx.packets,
            stats->proto_ext.udp_basic.tx.bytes);
    }
}
...
int main(int argc, char *argv[])
{
    ...
    rc = spdt_connect(stream_idx, &conn_params);
    if (rc)
        goto exit;
    is_connected = 1;
