...
static int tcpspdtest_retrieve_file_data(int8_t stream_idx, char *file_name)
{
 ...
 establish_session:
	...
	if (use_proxy || target.protocol[0] != 'f' /*not ftp[s]*/) {
		...
		if (0 == strcmp(G.fname_out, "tcpspdtest"))
        {
			...
			if (0 == spdt_connect(tcpspdtest_stream_idx, &params))
			{
				tcpspdtest_retrieve_file_data(tcpspdtest_stream_idx, target.path);
