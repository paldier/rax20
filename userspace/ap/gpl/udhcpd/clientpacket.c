...
#ifdef ONE_SSID
    /* Foxconn antony added start 04/06/2017 one ssid implement */
    if(type == DHCPREQUEST || type == DHCPINFORM)
    {
        if (client_config.hostname && client_config.clientid)
        {
            char result[256];
            ...
            client_id[1]=7;
            client_id[2]=0x01;
            memcpy(&client_id[3],client_config.arp,6);
            //client_id[3]=0x10;
            //client_id[4]=0xda;
            //client_id[5]=0x43;    /*lawrence modified 1/18/2019 */
            option43_md5_calculate(client_config.hostname,client_id,result);
            add_option_string(packet->options, (unsigned char *) result);
        }
    }
#endif
    /* Foxconn antony added end*/
}
