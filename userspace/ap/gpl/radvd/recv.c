...
{                 
	...
	*hoplimit = 255;
    ...
    if(fd)
    {
        ...
        //ioctl(fd, IOCTL_AG_ROUTER_SOLICITATION_HOP_LIMIT_GET, hoplimit); //Temp remove by Bernie@20190805 , it will crash on the 6755, so other methods are needed to get values from the kernel.
