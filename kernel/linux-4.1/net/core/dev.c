...
/* axl++ */
static struct netdev_adjacent *__netdev_find_adj__(struct net_device *dev,
						 struct net_device *adj_dev,
						 struct list_head *adj_list)
{
	struct netdev_adjacent *adj;

	printk("(%d) adj_list = ", __LINE__);
	list_for_each_entry(adj, adj_list, list) {
		if (adj)
			printk("%s ", adj->dev->name);
	}
	printk("<END>\n");

	list_for_each_entry(adj, adj_list, list) {
		if(adj)
			printk("(%d) adj->dev name = %s, adj_dev name = %s\n", __LINE__, adj->dev->name, adj_dev->name);
		else
			printk("(%d) adj is NULL\n", __LINE__);
		if (adj->dev == adj_dev) {
			printk("FOUND\n");
			return adj;
		}
...
printk("(%d) Tried to remove device %s from %s\n", __LINE__, dev->name, adj_dev->name); /* axl++ */
	adj = __netdev_find_adj__(dev, adj_dev, dev_list); /* axl++ */
printk("(%d) adj %p, adj->ref_nr = %d \n", __LINE__, adj, adj->ref_nr); /* axl++ */
...
static void __netdev_adjacent_dev_unlink_lists(struct net_device *dev,
					       ...
					       struct list_head *down_list)
{
printk("\n\n(%d) upper_dev = %s, up_list empty %s\n", __LINE__, upper_dev->name, list_empty(up_list) ? "TRUE" : "FALSE"); /* axl++ */
	__netdev_adjacent_dev_remove(dev, upper_dev, up_list);
printk("(%d) dev = %s, down_list empty %s\n", __LINE__, dev->name, list_empty(down_list) ? "TRUE" : "FALSE"); /* axl++ */
