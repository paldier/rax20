...
void dump_RDD_DDR_QUEUES_BASE_PTR(bdmf_session_handle session, unsigned char *p)
{
	unsigned int r;
	bdmf_session_print(session, "  Register DDR_QUEUES_BASE_PTR\n");
...
void dump_RDD_TCPSPDTEST_STREAM(bdmf_session_handle session, unsigned char *p)
{
	...
	MREAD_8((uint8_t *)p, r);
	bdmf_session_print(session, "\tTCPSPDTEST_ENGINE_CONN_INFO_l2_hdr_len= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_8((uint8_t *)p + 1, r);
	bdmf_session_print(session, "\tTCPSPDTEST_ENGINE_CONN_INFO_l3_hdr_len= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_8((uint8_t *)p + 2, r);
	bdmf_session_print(session, "\tTCPSPDTEST_ENGINE_CONN_INFO_l3_protocol= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_8((uint8_t *)p + 3, r);
	bdmf_session_print(session, "\tTCPSPDTEST_ENGINE_CONN_INFO_tx_hdr_len= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");
	...
	bdmf_session_print(session, "\tTCPSPDTEST_ENGINE_CONN_INFO_reserved0= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_16((uint8_t *)p + 8, r);
	bdmf_session_print(session, "\tTCPSPDTEST_ENGINE_CONN_INFO_wan_flow= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_16((uint8_t *)p + 10, r);
	bdmf_session_print(session, "\tTCPSPDTEST_ENGINE_CONN_INFO_tx_qm_queue= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");
	...
	FIELD_MREAD_8((uint8_t *)p + 111, 3, 5, r);
	bdmf_session_print(session, "\tTCPSPDTEST_ENGINE_TCB_reserved1= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	FIELD_MREAD_8((uint8_t *)p + 111, 2, 1, r);
	bdmf_session_print(session, "\tTCPSPDTEST_ENGINE_TCB_up_is_rx_win_stop= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	FIELD_MREAD_8((uint8_t *)p + 111, 1, 1, r);
	bdmf_session_print(session, "\tTCPSPDTEST_ENGINE_TCB_up_is_timeout= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	FIELD_MREAD_8((uint8_t *)p + 111, 0, 1, r);
	bdmf_session_print(session, "\tTCPSPDTEST_ENGINE_TCB_up_is_active= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_16((uint8_t *)p + 112, r);
	bdmf_session_print(session, "\tTCPSPDTEST_ENGINE_REF_PKT_HDR_size= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_16((uint8_t *)p + 114, r);
	bdmf_session_print(session, "\tTCPSPDTEST_ENGINE_REF_PKT_HDR_offset= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 116, r);
	bdmf_session_print(session, "\tTCPSPDTEST_ENGINE_REF_PKT_HDR_reserved= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 120, r);
	bdmf_session_print(session, "\tTCPSPDTEST_ENGINE_REF_PKT_HDR_hdr= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 124, r);
	bdmf_session_print(session, "\tTCPSPDTEST_ENGINE_REF_PKT_HDR_hdr_1= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 128, r);
	bdmf_session_print(session, "\tTCPSPDTEST_ENGINE_REF_PKT_HDR_hdr_2= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 132, r);
	bdmf_session_print(session, "\tTCPSPDTEST_ENGINE_REF_PKT_HDR_hdr_3= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 136, r);
	bdmf_session_print(session, "\tTCPSPDTEST_ENGINE_REF_PKT_HDR_hdr_4= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 140, r);
	bdmf_session_print(session, "\tTCPSPDTEST_ENGINE_REF_PKT_HDR_hdr_5= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 144, r);
	bdmf_session_print(session, "\tTCPSPDTEST_ENGINE_REF_PKT_HDR_hdr_6= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 148, r);
	bdmf_session_print(session, "\tTCPSPDTEST_ENGINE_REF_PKT_HDR_hdr_7= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 152, r);
	bdmf_session_print(session, "\tTCPSPDTEST_ENGINE_REF_PKT_HDR_hdr_8= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 156, r);
	bdmf_session_print(session, "\tTCPSPDTEST_ENGINE_REF_PKT_HDR_hdr_9= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 160, r);
	bdmf_session_print(session, "\tTCPSPDTEST_ENGINE_REF_PKT_HDR_hdr_10= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 164, r);
	bdmf_session_print(session, "\tTCPSPDTEST_ENGINE_REF_PKT_HDR_hdr_11= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 168, r);
	bdmf_session_print(session, "\tTCPSPDTEST_ENGINE_REF_PKT_HDR_hdr_12= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 172, r);
	bdmf_session_print(session, "\tTCPSPDTEST_ENGINE_REF_PKT_HDR_hdr_13= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 176, r);
	bdmf_session_print(session, "\tTCPSPDTEST_ENGINE_REF_PKT_HDR_hdr_14= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 180, r);
	bdmf_session_print(session, "\tTCPSPDTEST_ENGINE_REF_PKT_HDR_hdr_15= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 184, r);
	bdmf_session_print(session, "\tTCPSPDTEST_ENGINE_REF_PKT_HDR_hdr_16= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 188, r);
	bdmf_session_print(session, "\tTCPSPDTEST_ENGINE_REF_PKT_HDR_hdr_17= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 192, r);
	bdmf_session_print(session, "\tTCPSPDTEST_ENGINE_REF_PKT_HDR_hdr_18= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 196, r);
	bdmf_session_print(session, "\tTCPSPDTEST_ENGINE_REF_PKT_HDR_hdr_19= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 200, r);
	bdmf_session_print(session, "\tTCPSPDTEST_ENGINE_REF_PKT_HDR_hdr_20= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 204, r);
	bdmf_session_print(session, "\tTCPSPDTEST_ENGINE_REF_PKT_HDR_hdr_21= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 208, r);
	bdmf_session_print(session, "\tTCPSPDTEST_ENGINE_REF_PKT_HDR_hdr_22= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 212, r);
	bdmf_session_print(session, "\tTCPSPDTEST_ENGINE_REF_PKT_HDR_hdr_23= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 216, r);
	bdmf_session_print(session, "\tTCPSPDTEST_ENGINE_REF_PKT_HDR_hdr_24= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 220, r);
	bdmf_session_print(session, "\tTCPSPDTEST_ENGINE_REF_PKT_HDR_hdr_25= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 224, r);
	bdmf_session_print(session, "\tTCPSPDTEST_ENGINE_REF_PKT_HDR_hdr_26= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 228, r);
	bdmf_session_print(session, "\tTCPSPDTEST_ENGINE_REF_PKT_HDR_hdr_27= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 232, r);
	bdmf_session_print(session, "\tTCPSPDTEST_ENGINE_REF_PKT_HDR_hdr_28= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 236, r);
	bdmf_session_print(session, "\tTCPSPDTEST_ENGINE_REF_PKT_HDR_hdr_29= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_8((uint8_t *)p + 240, r);
	bdmf_session_print(session, "\tTCPSPDTEST_ENGINE_PKT_DROP_is_drop= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_8((uint8_t *)p + 241, r);
...
void dump_RDD_SPDTEST_GEN_CFG(bdmf_session_handle session, unsigned char *p)
{
	...
	FIELD_MREAD_32((uint8_t *)p + 0, 0, 23, r);
	bdmf_session_print(session, "\treserved                 = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");
