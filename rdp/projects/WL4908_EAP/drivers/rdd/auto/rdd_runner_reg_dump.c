...
{
	unsigned int r;
	bdmf_session_print(session, "  Register CAPWAPF_CFG_ENTRY\n");

	FIELD_MREAD_8((uint8_t *)p, 7, 1, r);
	bdmf_session_print(session, "\tclear_statistics         = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");
	...
	MREAD_16((uint8_t *)p + 2, r);
	bdmf_session_print(session, "\tmax_frame_size           = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");
...
{
	unsigned int r;
	bdmf_session_print(session, "  Register CAPWAPF_STATS_ENTRY\n");

	MREAD_32((uint8_t *)p, r);
	bdmf_session_print(session, "\tupstream_packets         = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 4, r);
	bdmf_session_print(session, "\tinvalid_ethtype_or_ip_header= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 8, r);
	bdmf_session_print(session, "\tinvalid_protocol         = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 12, r);
	bdmf_session_print(session, "\tinvalid_capwap_version_type= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 16, r);
	bdmf_session_print(session, "\tcongestion               = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 20, r);
	bdmf_session_print(session, "\tmiddle_fragments         = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 24, r);
	bdmf_session_print(session, "\tfirst_fragments          = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 28, r);
	bdmf_session_print(session, "\tlast_fragments           = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");
...
{
	unsigned int r;
	bdmf_session_print(session, "  Register CAPWAPR_CFG_ENTRY\n");
	...
	FIELD_MREAD_8((uint8_t *)p, 5, 1, r);
	bdmf_session_print(session, "\tipv4_window_check        = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");
	...
	MREAD_16((uint8_t *)p + 2, r);
	bdmf_session_print(session, "\treceive_frame_buffer_size= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");
...
{
	unsigned int r;
	bdmf_session_print(session, "  Register CAPWAPR_STATS_ENTRY\n");

	MREAD_32((uint8_t *)p, r);
	bdmf_session_print(session, "\tinvalid_headers          = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 4, r);
	bdmf_session_print(session, "\tpacket_reassemble_error  = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 8, r);
	bdmf_session_print(session, "\tfragments_received       = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 12, r);
	bdmf_session_print(session, "\tfragments_evicted        = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 16, r);
	bdmf_session_print(session, "\tunfragmented_packets     = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");
	...
	MREAD_32((uint8_t *)p + 32, r);
	bdmf_session_print(session, "\tpackets_not_in_window    = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 36, r);
	bdmf_session_print(session, "\tpackets_reassembled      = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 40, r);
	bdmf_session_print(session, "\tinvalid_fragment         = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 44, r);
	bdmf_session_print(session, "\treassembled_packet_too_big= 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");
...
{
	unsigned int r;
	bdmf_session_print(session, "  Register CAPWAPR_FLUSH_ENTRY\n");
...
void dump_RDD_CAPWAP_CFG_ENTRY(bdmf_session_handle session, unsigned char *p)
{
	...
	bdmf_session_print(session, "  Register CAPWAP_CFG_ENTRY\n");
	...
	MREAD_16((uint8_t *)p + 2, r);
	bdmf_session_print(session, "\tac_port                  = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 4, r);
	bdmf_session_print(session, "\tac_ip_address            = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 8, r);
	bdmf_session_print(session, "\tap_ip_address            = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	bdmf_session_print(session, "\tap_ethernet_mac_address  =\n\t");
	for (i=0,j=0; i<6; i++)
	{
		MREAD_I_8((uint8_t *)p + 12, i, r);
		bdmf_session_print(session, "0x%08x ", (unsigned int)r);
		j++;
		if (j >= 8)
		{
