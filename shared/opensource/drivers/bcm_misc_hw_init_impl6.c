...
#if defined(CONFIG_BCM963178) || defined(_BCM963178_)
    set_vreg_clk();
#elif defined(CONFIG_BCM963158) || defined(_BCM963158_)
#ifdef _CFE_
...
static void bcm_misc_hw_xmii_pads_init(void)
{
    ...
    for (n = 0; n < BP_MAX_ENET_MACS; n++)
    {
        for (i = 0; i < BP_MAX_SWITCH_PORTS; i++)
        {
            phy_id = Enet[n].sw.phy_id[i];
            port = i;
            if (IsRGMII(phy_id))
                goto found_rgmii;
        }
        ...
        {
            phy_id = Enet[n].sw.crossbar[i].phy_id;
            port = BP_CROSSBAR_PORT_BASE + i;
            if (IsRGMII(phy_id))
                goto found_rgmii;
        }
    }
...
found_rgmii:
    ...
    if (IsRGMII_1P8V(phy_id))
        GPIO->PadCtrl = GPIO->PadCtrl & ~MISC_XMII_PAD_MODEHV;
    else
        GPIO->PadCtrl = GPIO->PadCtrl | MISC_XMII_PAD_MODEHV;

    if (BpGetIfacePinmux (BP_PINMUX_FNTYPE_xMII | port, BP_PINMUX_MAX,  &n, &errcnt, Function, Muxinfo) != BP_SUCCESS)
        return;
    ...
    for (i = 0; i < n ;i++)
    {
        ...
        tp_data = 0;
        tp_data |= (6 /*14mA*/ << 12);                  // pad_sel
        tp_data |= ((IsRGMII_1P8V(phy_id)?1:0) << 15);  // pad_amp_en
        tp_data |= ((i < 6 ? 0 : 1) << 16);             // pad_ind - 0 for RX pads, 1 for TX pads
        tp_data |= ((IsRGMII_3P3V(phy_id)?1:0) << 17);  // pad_sel_gmii
...
#else
        arch_setup_dma_ops(rdp_dummy_dev, NULL, 0, NULL, false);
#endif
...
{
#if !defined(_BCM94908_) && !defined(CONFIG_BCM94908) && !defined(_BCM963158_)  && !defined(_BCM963178_) && !defined(_BCM947622_)
    bcm_ubus_config();
#endif
