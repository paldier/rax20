...
mac_type_t bp_parse_mac_type(const ETHERNET_MAC_INFO *emac_info, uint32_t port)
{
    ...
    {
    case BP_ENET_NO_PHY: // sysp
        if (port < 2) return MAC_TYPE_UNIMAC;
        break;
    case BP_ENET_EXTERNAL_SWITCH:   // sf2
        if (port < 4 || port == 5 || port == 8) return MAC_TYPE_SF2;
        break;
    }
...
{
    uint32_t id = emac_info->sw.phy_id[port];
    switch (emac_info->ucPhyType)
    {
    case BP_ENET_NO_PHY: // sysp
        return (void*)((IsPortConnectedToExternalSwitch(id)?UNIMAC_DRV_PRIV_FLAG_EXTSW_CONNECTED|UNIMAC_DRV_PRIV_FLAG_SHRINK_IPG:0));
    case BP_ENET_EXTERNAL_SWITCH: // sf2
        switch (port)
        {
        case 8: return (void *)(SF2MAC_DRV_PRIV_FLAG_IMP|SF2MAC_DRV_PRIV_FLAG_SHRINK_IPG);
        }
    }
    return (void *) 0;
}
