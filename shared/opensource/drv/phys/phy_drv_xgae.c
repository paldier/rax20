...
void sf2_xgae_lbe_op(phy_dev_t *phy_dev, laser_op_t lbe_op)
{
    int val;
...
int ethsw_xgae_light_detected(phy_dev_t *phy_dev)
{
    ...
    if (phy_serdes->serdes_status == SERDES_NOT_INITED) {
        ...
        ethsw_xgae_speed_set(phy_dev, speed, PHY_DUPLEX_FULL);
    }
...
static void wan_top_misc3_config(phy_serdes_t *phy_serdes)
{
    ...
    if (phy_serdes->sgmii_mode != NONSGMII_MODE)
...
static int serdes_enable (phy_serdes_t *phy_serdes, int serdes_rate_mode)
{
    ...
    //reg_model.wan_top_reg_blk.wan_top_misc_3_reg
    wan_top_misc3_config(phy_serdes);
...
int _ethsw_xgae_speed_set(phy_dev_t *phy_dev, phy_speed_t speed, phy_duplex_t duplex, int force)
{
    ...
    else if(phy_dev->cascade_next && phy_dev->cascade_next->phy_drv->phy_type == PHY_TYPE_EXT3 &&
            speed == PHY_SPEED_100)
        phy_serdes->sgmii_mode = SGMII_FORCE;
    else
        phy_serdes->sgmii_mode = NONSGMII_MODE;
    ...
    serdes_enable(phy_serdes, serdes_rate_mode);
    ...
    */
    ...
    error_cnt = error_cnt + serdes_microcode (phy_serdes->serdes_speed_mode, serdes_rate_mode);
    ...
    if (phy_serdes->sfp_module_type == SFP_COPPER)
        cascade_next->phy_drv->speed_set(cascade_next, speed, PHY_DUPLEX_FULL);
    ...
    if (phy_serdes->sgmii_mode != NONSGMII_MODE) {
        ...
        if (phy_serdes->sgmii_mode == SGMII_AUTO) {
...
/*
    Asynchronous register read needs software to take a guess
*/
static int asynchronous_pcsreg_read(phy_dev_t *phy_dev, int reg, int *val, int read_clear_mask)
{
#define ASYNC_READS 5   /* Small loop */
#define ASYNC_REPEATS 3 /* Big loop */
#define ASYNC_COMM   2  /* Successful reads among Small loop of ASYNC_READ times */
    int i, j;
    /* m: the most times of same value read, n: array index of most often read value; r: repeat times */
    int m = 0, n = 0, r = 0;    
    int v[ASYNC_READS]; /* read values */
    int c[ASYNC_READS]; /* How many times of the same value read */
    int read_clear_val = 0;

again:
    if (r++ > ASYNC_REPEATS) {
        /* We always read out different values, give up */
        printk("Failed to read a stable value of 10G PCS asynchronous bit register at %x\n", reg);
        printk("  - skip this round of handling and will try later. No worry.\n");
        return -1;
    }

    for (i=0; i<ASYNC_READS; i++)
        c[i] = 0;

    for (i=0; i<ASYNC_READS; i++) {
        readXgaePcsReg(DEVID_0 | LANE_0 | reg, &v[i]);
        read_clear_val |= v[i] & read_clear_mask; /* Save read&clear bits on every read */
        v[i] &= ~read_clear_mask;
    }

    for (i=0; i<ASYNC_READS; i++) {
        for (j=0; j<ASYNC_READS; j++) {
            if (v[j] == v[i])
                c[i]++;
        }
    }
    ...
    if (m < ASYNC_COMM) { /* All reads have different values, sleep and try again */
        msleep(1);
        goto again;
    }

    *val = v[n] | (read_clear_val & read_clear_mask);
    ...
    if (phy_serdes->sgmii_mode != NONSGMII_MODE) {
        ...
        switch(reg5 & ieee5_SgmiiSpeedMask) {
            case ieee5_SgmiiSpeed1G:
                phy_serdes->current_speed = phy_dev->speed = PHY_SPEED_1000;
                break;
            case ieee5_SgmiiSpeed100M:
                phy_serdes->current_speed = phy_dev->speed = PHY_SPEED_100;
                break;
            case ieee5_SgmiiSpeed10M:
                phy_serdes->current_speed = phy_dev->speed = PHY_SPEED_10;
                break;
        }
...
#if (CONFIG_BRCM_CHIP_REV == 0x63158A0)
            rxsm_state = rxc_state = rxc_latch = no_sticky_bits = in_rxc_state = 0; /* For compilation only */
            rxe_state = last_rxc_state = last_rxc_latch = last_rxsm_val = 0;  /* For compilation only */
            if(asynchronous_pcsreg_read(phy_dev, CL49_UserB0_rxsm_status, &val, 0)) {
                phy_dev->link = old_link;
                phy_dev->speed = old_speed;
                goto end;
            }

            r_type_code = val & r_type_coded_M;
            if ((val & lrxsm_latch_RX_C) == 0 || r_type_code == R_TYPE_E || r_type_code == R_TYPE_BAD) {
                printk("10G Serdes link down due to RXSM: %x\n", val);
                phy_dev->link = 0;
                goto end;
            }
#else
